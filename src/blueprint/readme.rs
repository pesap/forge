pub fn automated_update_section() -> &'static str {
    "## Automated Forge Syncs\n\nThis repository includes `.github/workflows/forge-sync.yaml`, which runs weekly and opens a PR with Forge-managed infrastructure syncs from `forge sync --path . --yes`. Forge reads the project configuration from `[tool.forge]` in `pyproject.toml` and does not use a separate status file. The workflow runs `uv lock` only when Forge changes lockfile-relevant `pyproject.toml` metadata.\n\nUse these commands for local maintenance:\n\n```bash\nforge sync --path . --dry-run\nforge sync --path . --check\nforge sync --path .\nuv lock\n```\n\nRun `uv lock` when Forge reports it as a next step after changing lockfile-relevant `pyproject.toml` metadata.\n\n"
}
...
mod tests {
    ...
    fn automated_update_section_uses_forge_managed_language() {
        ...
        assert!(
            section.contains("only when Forge changes lockfile-relevant `pyproject.toml` metadata")
        );
