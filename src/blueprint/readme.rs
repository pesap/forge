...
mod tests {
    ...
    fn automated_update_section_uses_forge_managed_language() {
        ...
        assert!(section.contains("Automated Forge Updates"));
        assert!(section.contains("Forge-managed infrastructure updates"));
        assert!(section.contains("forge update --path ."));
        assert!(section.contains("forge update --path . --dry-run"));
        assert!(section.contains("forge update --path . --check"));
