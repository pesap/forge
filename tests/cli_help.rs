...
#[test]
fn self_update_uses_standard_human_output() {
    let mut cmd = Command::cargo_bin("forge").expect("forge binary should build");
    ...
    cmd.args(["self", "update"])
        ...
        .stdout(contains("install path:"))
        .stdout(contains("status: manual update required"))
        .stdout(contains("$ install the latest release"));
}
